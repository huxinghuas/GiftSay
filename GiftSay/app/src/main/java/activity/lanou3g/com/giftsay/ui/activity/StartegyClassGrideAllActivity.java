 ...
 *  攻略页面->品类->查看全部
 */
public class StartegyClassGrideAllActivity extends AbsBaseActivity {
    ...
    protected void initDatas() {
        ...
        VolleyeInstance.getInstance().startRequest(GetUrl.STRATEGY_GRIDE, new VolleyResult() {
            ...
            public void success(String resultStr) {
                ...
                List<StartegyLvBean.DataBean.ChannelGroupsBean.ChannelsBean> datas = bean.getData().getChannel_groups().get(0).getChannels();
                adapter.setDatas(datas);
                Log.d("dd", "adapter:" + adapter);
                recyclerView.setAdapter(adapter);
                GridLayoutManager manager = new GridLayoutManager(StartegyClassGrideAllActivity.this,2);
                recyclerView.setLayoutManager(manager);
                titleTv.setText(bean.getData().getChannel_groups().get(0).getName());
            }
